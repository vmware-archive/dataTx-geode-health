 ...
 *
 */
public class GeodeART implements  Closeable
{
		...
		for(Map.Entry<String, ServerXmlGart> entry : this.serverXmlGartMap.entrySet())
		{
			...
			for (String id : ids)
			{
				if(!entry.getValue().isGatewaySenderEnableBatchConflation(id))
					throw new GartWarningException("GatewaySenderEnableBatchConflation","Gateway sender id:"+id+" does not have sender enabled batch conflation in file:"+entry.getKey());
			}
		}
	}//------------------------------------------------
	...
	public void assertXmlPathContains(String xpath, String containsText)
	{
		for(Map.Entry<String, ServerXmlGart> entry : this.serverXmlGartMap.entrySet())
		{
			...
			{
				throw new GartWarningException("Pdx"," xpath:"+xpath+" does not contain "+containsText+" file:"+entry.getKey()); 
			}
		}
		...
		for(Map.Entry<String, ServerXmlGart> entry : this.serverXmlGartMap.entrySet())
		{
			...
			for (String id : ids)
			{
				if(!entry.getValue().hasGatewaySenderSocketBucketSize(id))
					throw new GartWarningException("GatewaySenderSocketBucketSize","Gateway sender id:"+id+" does not set the socket-bucket-size file:"+entry.getKey());
			}
		}
	}//------------------------------------------------
	...
	public void assertCacheServerHasSocketBuffetSize()
	{
		for(Map.Entry<String, ServerXmlGart> entry : this.serverXmlGartMap.entrySet())
		{
			if(!entry.getValue().hasCachServerHasSocketBufferSize())
				throw new GartWarningException("CacheServerSocketSize","file:"+entry.getKey()+" does not set the cache server socket buffer size");
		}
	}//------------------------------------------------
	 ...
	 */
	public static GeodeART getInstance()
	{
		return gart;
	}//------------------------------------------------
	 ...
	 */
	public GartJmxAsserter getGartJmxAsserter()
	{
	...
	public void setJmxConnection(String host,int port)
	{
		synchronized (GeodeART.class)
		{
			...
			this.gartJmxAsserter = new GartJmxAsserter(jmx);
		}
	...
	private GartJmxAsserter gartJmxAsserter = null;
	private JMX jmx = null;
	private Email email = null;
	private static GeodeART gart = new GeodeART();
