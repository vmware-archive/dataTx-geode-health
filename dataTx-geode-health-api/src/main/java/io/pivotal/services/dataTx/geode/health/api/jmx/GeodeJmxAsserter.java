...
import io.pivotal.services.dataTx.geode.health.exception.GeodeHealthErrorException;
...
public class GeodeJmxAsserter
{

	private final JMX jmx;
	
	public GeodeJmxAsserter(JMX jmx)
	{
		this.jmx = jmx;
	}//------------------------------------------------
	...
	public void assertRegionNumberOfBucketsWithoutRedundancy(int maxNumWithout)
	{
		try
		{
			...
			for (ObjectName objectName : results)
			{
				...
				{
					throw new GeodeHealthErrorException("NumberOfBucketsWithoutRedundancy","Expected NumBucketsWithoutRedundancy "+maxNumWithout+" > "+numBucketsWithoutRedundancy+"  for "+objectName);
				}
			}
		}
	...
	public void assertCacheServerCountGreaterThan(int minCacheServerCnt)
	{
		try
		{
			...
			if(cacheServerCount < minCacheServerCnt)
				throw new GeodeHealthErrorException("CacheServerCount","cacheServerCount:"+cacheServerCount+" < "+minCacheServerCnt);
		}
	...
	public void assertSystemAttributeGreaterThanEqualTo(String attributeName, int lowerBound)
	{
		try
		{
			...
			if(value < lowerBound)
				throw new GeodeHealthErrorException(attributeName,attributeName+":"+value+" < "+lowerBound);
		}
		catch (MalformedObjectNameException | InstanceNotFoundException e)
		{
			throw new SystemException(e);
	...
	throws Exception
	{
		...
		for (ObjectInstance objectInstance : rows)
		{
			...
			if(onDiskPercentage > percentageLimit)
				throw new GeodeHealthWarningException("TotalOnlyOnDisk"," total of disk percentage:"+onDiskPercentage+"% > "+percentageLimit+"% totalOnlyOnDisk:"+totalOnlyOnDisk+"  systemRegionEntryCount:"+systemRegionEntryCount+" region:"+objectName);
		}
	}
	public void assertMemberAttributeLessThan(String attributeName, int limit)
	{
		try
		{
			...
			for (ObjectInstance objectInstance : rows)
			{
				...
				if(value >limit)
					throw new GeodeHealthWarningException(attributeName,attributeName+" "+value+" > "+limit+" for object "+objectName);
			}
		}
	...
	public void checkPercentageUsed(int percentage, long maxMemory, long usedMemory)
	{
		...
		if(usedMemory > (comparePercent))
		{
			...
			throw new GeodeHealthWarningException("MemoryUsed",error);
		}
	}//------------------------------------------------
		...
		try
		{
			...
			for (ObjectInstance objectInstance : rows)
			{
				...
				if(!isTrue.apply(objectInstance).booleanValue())
				{
					...
					if(isWarn)
						throw new GeodeHealthWarningException(functionDesc,msg);
					else
						throw new GeodeHealthErrorException(functionDesc,msg);
				}
			}
