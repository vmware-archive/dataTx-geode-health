 ...
 *
 */
public class ServerXmlGeode
{
	public ServerXmlGeode(File file)
	{
		try
		{
			this.file = file;
	...
	public void assertPartitionedRegionParallelGwSender()
	{
		...
		{
			throw new GeodeHealthWarningException("PartitionedRegionParallel","Found partitioned regions with not parallel gateway senders:"+violations+" in file:"+file);
		}
	}//------------------------------------------------
	@SuppressWarnings("unchecked")
	public void assertRegionStatisticsEnabled()
	{
			...
			if(!violations.isEmpty())
			{
				throw new GeodeHealthWarningException("RegionStatisticsEnabled","Statistics Enabled not enabled for the following regions:"+violations+" in file:"+file);
			}
		}//------------------------------------------------
	...
	public void assertDisableAutoDiskCompaction()
	{
		...
		if(!violations.isEmpty())
			throw new GeodeHealthWarningException("DisableAutoDiskCompaction","Disk store auto compaction is not disabled:"+violations+" in file:"+file);
	...
	public void assertEnableAllowForceCompaction()
	{
		...
		if(!violations.isEmpty())
			throw new GeodeHealthWarningException("EnableAllowForceCompaction","Disk store allow-force-compaction is not enabled:"+violations+" in file:"+file);
	...
	public synchronized static ServerXmlGeode getInstance()
	{
		if(gart == null)
			gart = new ServerXmlGeode(new File(Config.getProperty("gart.file")));
	...
	private static ServerXmlGeode gart = null;
